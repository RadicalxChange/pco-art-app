...
function getSelectors(contract: Contract, erc165 = false) {
  ...
  const selectors = signatures.reduce((acc, val) => {
    if (
      val !== "init(bytes)" &&
      (erc165 || val !== "supportsInterface(bytes4)")
    ) {
...
export function useNativeStewardLicenseInit(
  data: NativeStewardLicenseInit | undefined
): FacetInit | null {
  ...
  useEffect(() => {
    ...
    const contract = new Contract(
      nativeStewardLicenseFacetAddress[
        network.chain.id as keyof typeof nativeStewardLicenseFacetAddress
      ],
...
export function usePeriodicPCOParamsInit(
  data: PeriodicPCOParamsInit | undefined
): FacetInit | null {
  const [facetInit, setFacetInit] = useState<FacetInit | null>(null);
  ...
  useEffect(() => {
    ...
    const contract = new Contract(
      periodicPcoParamsFacetAddress[
        network.chain.id as keyof typeof periodicPcoParamsFacetAddress
      ],
...
export function useIDABeneficiaryInit(
  data: IDABeneficiaryInit | undefined
): FacetInit | null {
  const [facetInit, setFacetInit] = useState<FacetInit | null>(null);
  ...
  useEffect(() => {
    ...
    const contract = new Contract(
      idaBeneficiaryFacetAddress[
        network.chain.id as keyof typeof idaBeneficiaryFacetAddress
      ],
...
export function useEnglishPeriodicAuctionInit(
  data: EnglishPeriodicAuctionInit | undefined
): FacetInit | null {
  const [facetInit, setFacetInit] = useState<FacetInit | null>(null);
  ...
  useEffect(() => {
    ...
    const contract = new Contract(
      englishPeriodicAuctionFacetAddress[
        network.chain.id as keyof typeof englishPeriodicAuctionFacetAddress
      ],
...
export function useAllowlistInit(
  data: AllowlistInit | undefined
): FacetInit | null {
  const [facetInit, setFacetInit] = useState<FacetInit | null>(null);
  ...
  useEffect(() => {
    ...
    const contract = new Contract(
      allowlistFacetAddress[
        network.chain.id as keyof typeof allowlistFacetAddress
      ],
      AllowlistFacetABI
    );
    ...
    setFacetInit({
      ...
      initData: contract.interface.encodeFunctionData(
        ...
        [data.owner, data.allowAny, data.addresses]
      ) as `0x${string}`,
      selectors: getSelectors(contract),
    });
  }, [data]);
...
export function useAccessControlInit(
  data: AccessControlInit | undefined
): FacetInit | null {
  const [facetInit, setFacetInit] = useState<FacetInit | null>(null);
  ...
  useEffect(() => {
    ...
    const contract = new Contract(
      accessControlFacetAddress[
        network.chain.id as keyof typeof accessControlFacetAddress
      ],
