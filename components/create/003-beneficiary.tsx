...
export default function ConfigBeneficiaryFacet({ nextStep, prevStep }: { nextStep: () => void; prevStep: () => void }) {
  ...
  useEffect(() => {
    setValue('beneficiary.ida.token', network.chain ? ethXAddress[network.chain.id as keyof typeof ethXAddress] : undefined)
  }, [network])
    ...
    <div className="min-w-full rounded-md bg-neutral-100 p-4 text-center dark:bg-neutral-800">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="mb-6">
          ...
          <label htmlFor="cycle" className="mb-2 block text-sm font-medium text-gray-900 dark:text-white">
            Enter the addresses and allocation units to define your Creator Circle. You can enter a single Ethereum address if you have other
          ...
          <FormProvider {...methods}>
            ...
            )) ?? <CreatorCircleAllocationEntry index={0} register={register} totalUnits={totalUnits!} />}
          </FormProvider>
          <div className="mb-6 flex">
            <button
              className="btn btn-sm mx-1 grow bg-gradient-to-r from-emerald-500 to-emerald-400 text-white"
              onClick={() => {
                setValue(`beneficiary.allocation.${watchAllocation?.length ?? 0 + 1}.units`, 0)
              }}>
