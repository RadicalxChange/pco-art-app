...
function updateAction(state: GlobalState, payload: FormPayload) {
  ...
  return {
    ...
    allowlistInitData: {
      ...
      addresses: allowlistInput['allow-any'] === 'false' ? allowlistInput.addresses.map((v) => v as Address) : [],
    } as AllowlistInit,
  }
}

export default function ConfigAllowlistFacet({ nextStep, prevStep }: { nextStep: () => void; prevStep: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="min-w-full rounded-md bg-neutral-100 p-4 dark:bg-neutral-800">
        ...
        <div className="mb-6">
          <label htmlFor="allowlist.allow-any" className="mb-2 block text-sm font-medium text-gray-900 dark:text-white">
            Criteria
          </label>
          <label htmlFor="allowlist.allow-any" className="mb-2 block text-sm font-medium text-gray-900 dark:text-white">
            Set who can participate in this token&apos;s Auction Pitches.
          </label>
          <select {...register('allowlist.allow-any')} className="grow rounded-lg border-gray-600 dark:bg-gray-700">
        ...
        {watchAllowAny === 'false' && (
          <div className="mb-6">
            ...
            <FormProvider {...methods}>
              {watchAddresses?.map((_, index) => <AllowlistEntry key={index} index={index} register={register} />) ?? (
...
function AllowlistEntry({ index, register }: { index: number; register: UseFormRegister<FormPayload> }) {
