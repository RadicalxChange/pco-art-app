...
export function NetworkStatus({ className }: any) {
  ...
  const explorerUrl = network.chain?.blockExplorers?.default.url ?? publicClient.chain?.blockExplorers?.default.url
  ...
  const classesBadge = classNames(
    ...
    `bg-${GetNetworkColor(network.chain?.network ?? publicClient.chain?.network)}-200`,
    `text-${GetNetworkColor(network.chain?.network ?? publicClient.chain?.network)}-700 dark:text-${GetNetworkColor(
      network.chain?.network ?? publicClient.chain?.network
    )}-700 py-2`
  )
  ...
  return (
    <div className={classes}>
      <span className={classesBadge}>
        <span className="px-1">{network.chain?.name ?? publicClient.chain?.name ?? 'Optimism'}</span>
      </span>
      {explorerUrl && (
