...
type Subscription = { subscriber: { id: string }; units: string; totalAmountReceivedUntilUpdatedAt: string; approved: boolean }

export default function CreatorCirclePage({ params }: { params: { 'token-address': string } }) {
  ...
  const totalDistribution = creatorCircle?.indexes[0]?.totalAmountDistributedUntilUpdatedAt ?? null
  const superToken = creatorCircle?.indexes[0]?.token.id ?? null
  const indexId = creatorCircle?.indexes[0]?.indexId ?? null

  useEffect(() => {
    ;(async () => {
      if (!provider || !chain || !creatorCircle?.indexes[0] || !account.address || !signer || isApproving || isWithdrawing) {
        return
      }
      ...
      const nativeSuperToken = await sfFramework.loadNativeAssetSuperToken(superToken)
      const superTokenBalance = await nativeSuperToken.balanceOf({ account: account.address, providerOrSigner: signer })
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      const nativeSuperToken = (await sfFramework.loadSuperToken(superToken)) as NativeAssetSuperToken
      const op = nativeSuperToken.downgrade({ amount: superTokenBalance ?? '0' })
  ...
  const calcAmountReceived = (
    ...
    const amountReceived = subscriberTotalAmountReceivedUntilUpdatedAt + (publisherIndexValue - subscriberIndexValueUntilUpdatedAt) * subscriberUnits
  ...
  return (
    <div className="w-10/12 lg:w-9/12">
      ...
      <table className="mt-4 w-full border-separate border-spacing-4">
        ...
        <tbody>
          {creatorCircle?.indexes[0]?.subscriptions.map((subscription: Subscription, i: number) => (
            <tr key={i} className="">
              <td className="rounded-lg bg-gray-300 p-2 dark:bg-gray-700">{truncateStr(subscription.subscriber.id, 20)}</td>
              <td className="rounded-lg bg-gray-300 p-2 dark:bg-gray-700">{subscription.units}</td>
              <td className="rounded-lg bg-gray-300 p-2 dark:bg-gray-700">
                {parseFloat(((Number(subscription.units) * 100) / totalUnits).toFixed(2))}
              </td>
              <td className="rounded-lg bg-gray-300 p-2 dark:bg-gray-700">{amountsReceived[i] ? amountsReceived[i] : '0'}</td>
              {subscription.subscriber.id === account.address?.toLowerCase() && !subscription.approved ? (
                <td className="flex justify-center">
                  <button className="w-28 rounded-lg bg-green-500 px-5 py-2 font-bold lg:w-40" onClick={handleClaim}>
                    {isApproving ? (
      ...
      {account?.address ? (
        <div className="mt-12">
          ...
          <p className="mt-2 leading-6">
            The Honorariums from all Creator Circles in which you have approved units are collected in the single balance shown below.
            <br /> You can utilize this{' '}
            <Link href="https://docs.superfluid.finance/superfluid/developers/super-tokens" target="_blank" prefetch={false} className="underline">
              Super Token
            </Link>{' '}
            balance as is or withdraw to ETH.
          </p>
          <h5 className="mt-5 text-lg font-bold">{superTokenBalance ? formatEther(BigInt(superTokenBalance)).slice(0, 10) : 0} ETHx</h5>
          {superTokenBalance && BigInt(superTokenBalance) > 0 ? (
            <button className="mt-3 w-48 rounded-lg bg-green-500 px-5 py-2 font-bold" onClick={handleWithdraw}>
              {isWithdrawing ? <span className="lds-dual-ring" /> : 'Withdraw Funds'}
            </button>
          ) : null}
        </div>
      ) : null}
