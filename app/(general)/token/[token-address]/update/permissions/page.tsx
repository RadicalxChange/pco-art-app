...
import { accessControlFacetABI, nativeStewardLicenseFacetABI } from '@/lib/blockchain'
import { ADMIN_ROLE, useGetRoleMember, useHasRole } from '@/lib/hooks/use-access-control-facet'
...
type Action = {
  functionName: 'transferOwnership' | 'grantRole' | 'revokeRole' | 'renounceRole'
...
export default function UpdatePCOSettingsPage({ params }: { params: { 'token-address': string } }) {
  ...
  const [isAcceptingOwnership, setIsAcceptingOwnership] = useState<boolean>(false)
  ...
  const { hasAllowlistRole, hasAuctionRole, hasBeneficiaryRole, hasAddTokenToCollectionRole, hasPcoParamsRole, hasAdminRole } = useHasRole({
    tokenAddress,
    accountAddress: account.address,
    watch: true,
  })
  const roleMember = useGetRoleMember({ tokenAddress, index: BigInt(0), watch: true })

  const tokenName = data && data[0].status === 'success' ? data[0].result : null
  const collectionOwner = data && data[1].status === 'success' ? data[1].result : null
  const nomineeOwner = data && data[2].status === 'success' ? data[2].result : null
  ...
  useEffect(() => {
    ...
    setValue('permissions.addTokenToCollection', roleMember.addTokenToCollection)
    ...
    const subscription = watch((value, { name, type }) => {
      ...
      if (owner?.toLowerCase() !== collectionOwner?.toLowerCase() || (isTransferPending && owner?.toLowerCase() !== nomineeOwner.toLowerCase())) {
        actions.push({ functionName: 'transferOwnership', abi: ownableDiamondABI, args: [owner] })
      }

      if (roleAdmin?.toLowerCase() !== roleMember.adminRole?.toLowerCase()) {
        actions.push({ functionName: 'grantRole', abi: accessControlFacetABI, args: [ADMIN_ROLE, roleAdmin] })
        actions.push({ functionName: 'renounceRole', abi: accessControlFacetABI, args: [ADMIN_ROLE] })
      }

      for (const key of permissionsKeys) {
        if (permissions[key]?.toLowerCase() !== roleMember[key]?.toLowerCase()) {
          actions.push(
            { functionName: 'revokeRole', abi: accessControlFacetABI, args: [keccak256(toHex(roles[key])), roleMember[key] ?? '0x'] },
            { functionName: 'grantRole', abi: accessControlFacetABI, args: [keccak256(toHex(roles[key])), permissions[key]] }
          )
        }
      }
  ...
  return (
    <div className="m-auto w-2/4">
      ...
      <h2 className="text-medium mt-5 mb-2 text-2xl font-bold">Edit Permissions</h2>
      <p>
        ...
        Stewards & Creator Circle. There are social and security trade-offs with upgradability vs. immutability.
        <br />
        <br />
        You can forgo, maintain, or allocate these permissions. We've set suggested defaults. Make sure secure access to the selected addresses can be
      ...
      <p>
        ...
        address can change a token's PCO settings, implementation/configuration of core components, and reassign the roles below.
      </p>
      <div className="mt-5 flex flex-col gap-2 lg:flex-row lg:items-center">
        <input
          ...
          style={{
            pointerEvents: account.address === collectionOwner ? 'auto' : 'none',
            opacity: account.address === collectionOwner ? 1 : 0.6,
          }}
        />
        {isTransferPending && nomineeOwner === account.address ? (
          <button className="rounded-full bg-blue-500 py-3 lg:w-60" onClick={handleTransferAcceptance}>
            {isAcceptingOwnership ? <span className="lds-dual-ring" /> : <span className="font-bold">Accept Ownership</span>}
          </button>
        ) : isTransferPending ? (
      ...
      <p>Assign the ability to configure the details of each core component. Token Admins can reassign these roles at any time.</p>
      <form onSubmit={(e) => e.preventDefault()}>
        <div
          ...
          <label htmlFor="cycle" className="mb-2 block font-medium text-gray-900 dark:text-white">
            Role Admin
          </label>
          <input
            {...register('roleAdmin')}
        ...
        <BranchIsWalletConnected>
          <button
            ...
            {isSaving ? (
              <>
                <span className="lds-dual-ring" /> {completedActions + 1}/{actions.length}
              </>
            ) : (
              <span>Save {actions.length > 0 ? `(${actions.length})` : null}</span>
            )}
          </button>
