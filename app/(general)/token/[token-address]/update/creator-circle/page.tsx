...
import { idaBeneficiaryFacetABI, nativeStewardLicenseFacetABI } from '@/lib/blockchain'
import { truncateStr } from '@/lib/utils'

export default function UpdateCreatorCirclePage({ params }: { params: { 'token-address': string } }) {
  ...
  const tokenName = tokenInfo && tokenInfo[0].status === 'success' ? tokenInfo[0].result : null
  const owner = tokenInfo && tokenInfo[1].status === 'success' ? tokenInfo[1].result : null
  let totalUnits = watcher?.reduce((acc: number, curr: { subscriber: string; units: string }) => acc + Number(curr.units), 0)
  ...
  useEffect(() => {
    ...
    const creatorCircle = currentCreatorCircle.indexes[0].subscriptions.map((subscription: { subscriber: { id: string }; units: string }) => {
      return {
  ...
  return (
    <div className="m-auto w-2/4">
      ...
      <h2 className="text-medium mt-5 text-2xl font-bold">Edit Creator Circle</h2>
      <label htmlFor="cycle" className="mt-12 mb-2 block text-sm font-medium text-gray-900 dark:text-white">
        The Creator Circle is the group of people/organizations that receive a token&apos;s Periodic Honorarium.
      </label>
      <div className="mb-6">
        <label htmlFor="cycle" className="mb-2 block text-sm font-medium text-gray-900 dark:text-white">
          Allocation Table
        </label>
        <label htmlFor="cycle" className="mb-5 block text-sm font-medium text-gray-900 dark:text-white">
          Enter the addresses and allocation units to define your Creator Circle. You can enter a single Ethereum address if you have other mechanisms
          planned for allocation.
        </label>
        <FormProvider {...form}>
          {watcher?.map((_, index) => (
            <CreatorCircleAllocationEntry key={index} index={index} register={register} totalUnits={totalUnits} />
          ))}
        </FormProvider>
        <div className="mb-6 flex">
          <button
            ...
            onClick={() => {
              setValue(`beneficiary.allocation.${watcher?.length ?? 0 + 1}.units`, '0')
            }}>
